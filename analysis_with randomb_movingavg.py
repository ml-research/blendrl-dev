...
def run_random_baseline(env_name="ALE/Seaquest-v5", num_episodes=100):
    env = OCAtari(
        env_name=env_name,
        mode="ram",
        obs_mode="ori",
...
def process_and_plot_length_return(directory_path, target_tag, filename, compute_stats=False, baseline_value=None):
    ...
    if compute_stats:
        ...
        legend_labels.append(f'Mean: {mean_val:.2f} Â± {std_val:.2f}')
...
process_and_plot_length_return(directory_path, "charts/episodic_game_return", "episodic_game_return.csv", compute_stats=True)
